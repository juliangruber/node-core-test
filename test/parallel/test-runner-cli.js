...
for (const isolation of ['none', 'process']) {
...

  {
    ...
    const args = [common.testRunnerPath, '--test', '--test-reporter=tap',
    `--experimental-test-isolation=${isolation}`];
...

  {
    ...
    const args = [common.testRunnerPath, '--test', `--experimental-test-isolation=${isolation}`];
...

  {
    ...
    const args = [common.testRunnerPath, '--no-warnings',
...

  {
    ...
    const args = [common.testRunnerPath, '--test', '--test-shard=1', join(testFixtures, 'index.js')];
    const child = spawnSync(process.execPath, args, { cwd: testFixtures });
  ...
  {
    // --test-shard option validation
    const args = [common.testRunnerPath, '--test', '--test-shard=1/2/3', join(testFixtures, 'index.js')];
    const child = spawnSync(process.execPath, args, { cwd: testFixtures });
  ...
  {
    // --test-shard option validation
    const args = [common.testRunnerPath, '--test', '--test-shard=0/3', join(testFixtures, 'index.js')];
    const child = spawnSync(process.execPath, args, { cwd: testFixtures });
