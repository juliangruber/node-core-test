...
describe('node:test reporters', { concurrency: true }, () => {
  it('should default to outputing TAP to stdout', async () => {
    const child = spawnSync(process.execPath, [testFile, '--test']);
  ...
  it('should default destination to stdout when passing a single reporter', async () => {
    const child = spawnSync(process.execPath, [testFile, '--test', '--test-reporter', 'dot']);
    assert.strictEqual(child.stderr.toString(), '');
  ...
  it('should throw when passing reporters without a destination', async () => {
    const child = spawnSync(process.execPath, [testFile, '--test', '--test-reporter', 'dot', '--test-reporter', 'tap']);
  ...
  it('should throw when passing a destination without a reporter', async () => {
    const child = spawnSync(process.execPath, [testFile, '--test', '--test-reporter-destination', 'tap']);
  ...
  it('should support stdout as a destination', async () => {
    const child = spawnSync(process.execPath,
                            [testFile, '--test', '--test-reporter', 'dot', '--test-reporter-destination', 'stdout']);
    assert.strictEqual(child.stderr.toString(), '');
    assert.match(child.stdout.toString(), /\.XX\.\n/);
    assert.match(child.stdout.toString(), /Failed tests:/);
    assert.match(child.stdout.toString(), /✖ failing/);
  ...
  it('should support stderr as a destination', async () => {
    const child = spawnSync(process.execPath,
                            [testFile, '--test', '--test-reporter', 'dot', '--test-reporter-destination', 'stderr']);
  ...
  it('should support a file as a destination', async () => {
    ...
    const child = spawnSync(process.execPath,
                            [testFile, '--test', '--test-reporter', 'dot', '--test-reporter-destination', file]);
    assert.strictEqual(child.stderr.toString(), '');
    assert.strictEqual(child.stdout.toString(), '');
  ...
  it('should disallow using v8-serializer as reporter', async () => {
    const child = spawnSync(process.execPath, [testFile, '--test', '--test-reporter', 'v8-serializer']);
    assert.strictEqual(child.stdout.toString(), '');
  ...
  ['js', 'cjs', 'mjs'].forEach((ext) => {
    it(`should support a '${ext}' file as a custom reporter`, async () => {
      ...
      const child = spawnSync(process.execPath,
                              [common.testRunnerPath, testFile, '--test', '--test-reporter', fixtures.fileURL('test-runner/custom_reporters/', filename),
                               ]);
      assert.strictEqual(child.stderr.toString(), '');
  ...
  it('should throw when reporter setup throws asynchronously', async () => {
    const child = spawnSync(
      process.execPath,
      ['reporters.js', common.testRunnerPath, '--test', '--test-reporter', fixtures.fileURL('empty.js')],
  ...
  it('should throw when reporter errors asynchronously', async () => {
    const child = spawnSync(process.execPath,
                            [fixtures.path('test-runner/default-behavior/index.test.js'), '--test', '--test-reporter',
  ...
  it('should support stdout as a destination with spec reporter', async () => {
    ...
    const child = spawnSync(process.execPath,
                            [testFile, '--test', '--test-reporter', 'spec', '--test-reporter-destination', file]);
    assert.strictEqual(child.stderr.toString(), '');
    assert.strictEqual(child.stdout.toString(), '');
